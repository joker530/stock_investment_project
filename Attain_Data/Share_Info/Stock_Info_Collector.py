...
class Stock_Info_Collector():  # 包含一个SubPortfolio对象和方法
    ...
    def get_trade_date(self):
        ...
        trade_date.to_csv(Base_dir + '/Datasets/Data_Table/Trading_Timetable/' + 'stock_trade_cal.csv')
        # trade_date.to_csv("D:/量化投资/交易框架的编写/trade_cal.csv")  # 重新更新该数据表
        # return trade_dateS

    # 获取某一只股票的日、周、月线数据,但是不能获取指数数据
    def get_Kline_datas(self, period='daily', start_date="20220101", end_date="20230421", adjust="qfq"):
        # period='daily'; choice of {'daily', 'weekly', 'monthly'}
        df = ak.stock_zh_a_hist(self.code, period, start_date, end_date, adjust)
        self.KL = df
        df.to_csv(Base_dir + '/Datasets/Data_Table/K_Lines/datas_daily/{}.csv'.format(self.code1))
    ...
    def get_minute_datas_two(self, start_date="2021-01-01 09:32:00", end_date="2021-09-06 09:32:00", period='1',
                             adjust="hfq"):
        df = ak.stock_zh_a_hist_min_em(symbol=self.code, start_date=start_date, end_date=end_date, period=period,
                                       adjust=adjust)
...
if __name__ == "__main__":
    sic = Stock_Info_Collector()
    df = sic.get_Kline_datas()
    pass
