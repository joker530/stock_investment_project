...
def dynamic_import(module_name):  # 这个函数用来动态的导入模块
    try:
...
class BacktestFrame(wx.Frame):
    def __init__(self, *args, **kw):
        ...
        self.frequency = wx.ComboBox(panel, choices=["day", "minute", "tick"])
        hbox1.Add(wx.StaticText(panel, label="频率"), flag=wx.RIGHT, border=5)
        hbox1.Add(self.frequency, flag=wx.RIGHT, border=5)

        self.strategy = wx.ComboBox(panel, choices=["海龟交易算法", "SVM", "神经网络选股"])
    ...
    def _get_dates(self):  # 得到的都是datetime.datetime对象,一般设置在点击开始回测按钮后开始触发
        ...
        # 将两个变量转换为datetime.datetime对象,先暂时不转化了
    ...
    def execute(self, event):  # 定义当点击开始回测按钮后的执行函数
        ...
        context = Context(strategy=strategy, cash=mother_money, start_date=start_date, end_date=end_date,
                          freq=frequency)  # 此时这个上下文会自动生成一个总账户
        self.context = context
        # 从这里开始后要分成两个线程执行了，尝试用队列的方法进行两个进程中的通信。
        update_queue = mp.Queue()  # 创建一个队列用于进程间的通信
        end_event = mp.Event()  # 创建一个事件对象
        strategy_process = mp.Process(target=context.execute_strategy, args=(update_queue,))
