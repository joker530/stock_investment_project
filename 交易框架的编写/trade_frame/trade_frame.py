...
def ts_pro():
    ...
    ts.set_token('7d4004cd3c624ea07aa62334c00e65aadadeb2d472bfee5bfb4c1798')
    # 初始化pro接口
    pro = ts.pro_api()
    return pro
...
# %%下载并保存交易日的数据
ts_trade_cal(exchange='SSE', start_date='20200101', end_date='20230405')  #可以自行更改
...
class Context:
    def __init__(self, cash, start_date, end_date, trade_cal):
        ...
        self.dt = dateutil.parser.parse(str(last_tradeday))
con1 = Context(1000, '20230101', '20230405', trade_cal)
# %%实例化
class G:
    pass
...
#g = G()
# 实例化上下文数据Context
# 初始资金cash=100000，开始时间2022-03-01，结束时间2023-03-10
context = Context(100000, '20220301', '20230310', trade_cal)
# %% 实例化对象函数
def generate_example(cash=100000,start_date='20220301',end_date='20230310'):
    g=G()
    ts_trade_cal(exchange='SSE', start_date='20200101', end_date='20230405')  #可以自行更改
    trade_cal = pd.read_csv('G:\\ai\\量化投资\\trade_cal.csv')
...
# %%在一段时间的历史交易日中，获取历史数据
def attribute_history(security, count, trade_cal,context=context,fields=('open', 'close', 'high', 'low', 'vol')):
    end_date = (context.dt - timedelta(days=1)).strftime('%Y-%m-%d')
    start_date=trade_cal[(trade_cal['is_open'] == 1) & (trade_cal['cal_date'] <= str(end_date))][-count:].iloc[0,:]['cal_date']
...
def attribute_daterange_history(security, start_date, end_date, fields=('open', 'close', 'high', 'low', 'vol')):#vol是成交量的意思，以手为单位
    # 尝试读取本地数据
    try:
        ...
        if ((start_date<=str(df.index[-1])) & (start_date>=str(df.index[0])) & (end_date<=str(df.index[-1])) & (end_date>=str(df.index[0]))):  
