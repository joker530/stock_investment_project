...
class stock_plot(object):
    ...
    def datas_day_op(self, filename:'002230.SZ.csv'):
        data = pd.read_csv("G:/ai/量化投资/交易框架的编写/backtesting_platform/datas_table/datas_daily/"+filename)
        # 获取必要的数据
        ...
        data['upper_lim'] = np.round(data['pre_close'] * 1.10)  #上轨线
        data['lower_lim'] = np.round(data['pre_close'] * 0.90)  #下轨线
        data['average'] = np.round((data['high'] + data['low']) / 2) #日平均价格
        data['macd_dif'], data['macd_dea'], data['macd_m'] = np.round(talib.MACD(data['close'], fastperiod=12,slowperiod=26, signalperiod=9),2)
    ...
    def datas_year_op(self):
        pass
    def panel_init(self):
        plot_data = self.data   #先处理好数据后才能开始进行绘画操作
...
if __name__ == "__main__":
    candle = stock_plot()
    candle.datas_day_op('002624.SZ.csv')
    candle.panel_init()
